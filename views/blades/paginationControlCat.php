...
if($lastPage != 1){
	...
	$previousPage = $currentPage - 1; //Goes back one page
		$paginationControls .= '<a href="'.$category.'&pn='.$previousPage.'">Previous Page</a>';
		for($i = $currentPage-4; $i < $currentPage; $i++){ //Left side of pages
			if($i > 0){
			  $paginationControls .= '<a href="'.$category.'&pn='.$i.'">'.$i.'</a>';//Generates page numbers
			}
		}
	}
	...
	for($i = $currentPage+1; $i <= $lastPage; $i++){ // Right side of pages
		$paginationControls .= '<a href="'.$category.'&pn='.$i.'">'.$i.'</a>';
		if($i >= $currentPage+4){
	...
	if ($currentPage != $lastPage) {
		...
		$paginationControls .= '<a href="'.$category.'&pn='.$nextPage.'">Next Page</a>';
	}
}
?>
